...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSY QOUPAY.ID.                [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "qoupay" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke Qoupay.id${NC}"
    echo -e "${YELLOW}YOUTUBE : AC TV THE GAMING${NC}"
    echo -e "${YELLOW}WHATSAPP : 6285210868214${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©Qoupay.id${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by QOUPAY.ID.   ${NC}"
